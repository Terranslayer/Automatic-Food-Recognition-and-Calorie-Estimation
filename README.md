...
![Tests](https://img.shields.io/badge/tests-94%20pytest-green.svg)
...
✅ **Comprehensive Evaluation**: 94 unit tests, extensive metrics
...
│   └── report.md                 # QA report template
├── scripts/
│   ├── generate_results_table.py # Paper tables generator
│   ├── dataset_stats.py          # Dataset statistics
│   └── estimate_resources.py     # Resource estimation
...
pytest tests/test_training.py -v    # Training infrastructure tests
pytest tests/test_dataset.py -v     # Data pipeline tests
...
Evaluated on the Nutrition5k test split (676 dishes) in December 2025. Full numbers, including protein / carb / fat MAE and R², are in [`results/final_results_summary.json`](results/final_results_summary.json).

| Task | Model | Metric | Value |
|---|---|---|---|
| Classification (154 classes) | EfficientNet-B0, 5M params | top-1 / top-5 accuracy | 64.1% / 84.3% |
| Classification (154 classes) | ViT-B/16, 86M params | top-1 / top-5 accuracy | 42.2% / 74.4% |
| Calorie estimation | Direct regression: EfficientNet-B0 features + MLP | MAE | 94.3 kcal |
| Calorie estimation | Two-stage: category + mass regression | MAE | 136.0 kcal |
| Calorie estimation | Category-mean baseline | MAE | 153.4 kcal |
| Mass estimation | Direct regression | MAE | 57 g |

**What did not work.** The end-to-end multi-task model (Mask R-CNN → classifier → regressor) trains but evaluates badly: it learns *total-dish* calories from a single ground-truth box, while inference sums *per-instance* predictions, so calories are over-counted in proportion to the number of detections. Fixing normalization and adding gradient clipping did not change that. Nutrition5k has no per-instance labels, so the fix needs a different training objective rather than more epochs. Direct regression is the recommended model.
...
- ✅ Phase 5: QA & Evaluation Complete
