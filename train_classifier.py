...
def train_epoch(model, train_loader, criterion, optimizer, device, metrics, logger, epoch, config):
    """Train for one epoch."""
    model.train()
    metrics.reset()
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            top1_acc = correct / labels.size(0)
            ...
            metrics.update('top1_acc', top1_acc)
            metrics.update('top5_acc', top5_acc)

        # Update progress bar
        pbar.set_postfix({
            'loss': f"{metrics.get_average('loss'):.4f}",
            'top1': f"{metrics.get_average('top1_acc'):.4f}",
            'top5': f"{metrics.get_average('top5_acc'):.4f}"
        })
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            logger.log_scalar('train/top1_acc', top1_acc, global_step)
            logger.log_scalar('train/top5_acc', top5_acc, global_step)

    return metrics.get_averages()


def validate(model, val_loader, criterion, device, metrics, logger, epoch):
    """Validate the model."""
    model.eval()
    metrics.reset()
    ...
    logger.log_scalar('val/top1_acc', val_metrics['top1_acc'], epoch)
    logger.log_scalar('val/top5_acc', val_metrics['top5_acc'], epoch)
...
def main():
    ...
    if args.debug:
        ...
        print(" "*20 + "FOOD CLASSIFICATION TRAINING")
    print("="*80)
    ...
    try:
        for epoch in range(start_epoch, config['training']['num_epochs']):
            ...
            print(f"  Train - Loss: {train_results['loss']:.4f}, "
                  f"Top1: {train_results['top1_acc']:.4f}, "
                  f"Top5: {train_results['top5_acc']:.4f}")
            print(f"  Val   - Loss: {val_results['loss']:.4f}, "
                  f"Top1: {val_results['top1_acc']:.4f}, "
                  f"Top5: {val_results['top5_acc']:.4f}")
            ...
            checkpoint_manager.save_checkpoint(
                ...
                metrics={
                    ...
                    'val_top1_acc': val_results['top1_acc'],
